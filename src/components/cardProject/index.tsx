...
const Project = ({ name, description, img, direction, url }: TypedCard) => {
    ...
    const renderImg = (img : string) => {
        switch (img) {
            case "NuKenzie": return NuKenzie;
            case "HotelPet": return HotelPet;
            case "KenzieHub": return KenzieHub;
            case "Hamburgueria": return Hamburgueria;
            default: return '';
        }
    }
    return (
        <LiWrapper direction={direction} >
            ...
            <div className="content">
                <h2>{name}</h2>
                <p>{description}</p>
                <a target="_blank" rel="noreferrer" href={url}><button>Access</button></a>
            </div>
